...
func initializer{syscall_ptr: felt*, pedersen_ptr: HashBuiltin*, range_check_ptr}(
    ...
    recipient_address.write(recipient_contract);
...
@view
func getRecipientAddress{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}() -> (
    rep_address: felt
) {
    let (rep_address: felt) = recipient_address.read();
    return (rep_address,);
}
...
@view
func getMintPrice{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}() -> (
    mintPrice: Uint256
) {
    let (mintPrice: Uint256) = nft_mint_price.read();
    return (mintPrice,);
}
...
func owner{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}() -> (
    ...
    let (owner_contract_address: felt) = Ownable.owner();
...
func setMintPrice{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(
    newMintPrice: Uint256
) {
    ReentrancyGuard._start();
    Proxy.assert_only_admin();
    nft_mint_price.write(newMintPrice);
    ReentrancyGuard._end();
    return ();
}
...
func mint{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}() {
    ...
    ReentrancyGuard._start();
    let (phase: felt) = sale_phase.read();
    with_attr error_message("Not Mint Phase.") {
        assert phase = 1;
    }
...
@external
func changeOwner{pedersen_ptr: HashBuiltin*, syscall_ptr: felt*, range_check_ptr}(new_owner: felt) {
    alloc_locals;
    ReentrancyGuard._start();

    Ownable.assert_only_owner();
    Ownable.transfer_ownership(new_owner);
    owner_address.write(new_owner);
